...
class VOT_SOI(object):
    ...
    def __init__(self, root_dir, version=2017, anno_type='rect',
                 download=False, return_meta=False, list_file=None):
        super(VOT_SOI, self).__init__()
        assert version in self.__valid_versions, 'Unsupport VOT version.'
        assert anno_type in ['default', 'rect'], 'Unknown annotation type.'

        root_dir = root_dir.replace('VOT2016', 'VOT' + str(version))
        self.root_dir = root_dir
        self.version = version
        self.anno_type = anno_type
        self.name = 'VOT' + str(version)
        if download:
        ...
        if list_file is None:
            ...
            list_file = os.path.join('/mnt/second/wangyipei/SOI/data/VOT2019/most.json')
        self._check_integrity(root_dir, version, list_file)

        if list_file.endswith('json'):
            with open(list_file) as f:
                self.seq_names = json.load(f)
                self.seq_names = self.seq_names['test']
        else:
