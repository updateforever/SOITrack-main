...
class VideoCube(object):
    ...
    def __init__(self, root_dir, subset, version):
        ...
        if subset in ['train', 'val', 'test']:
            ...
            self.restart_files = [os.path.join(root_dir, 'attribute', 'restart', '{}.txt'.format(s)) for s in
                                  self.seq_names]
            self.absent_files = [os.path.join(root_dir, 'attribute', 'absent', '{}.txt'.format(s)) for s in
                                  self.seq_names]
    ...
    def __getitem__(self, index):
        ...
        absent_flag = np.loadtxt(self.absent_files[index])

        return img_files, anno, restart_flag, absent_flag
