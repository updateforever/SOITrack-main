...
class ExperimentVOT(object):
    ...
    def __init__(self, root_dir, save_dir, version=2016,
        ...
        self.result_dir = os.path.join(save_dir, 'VOT' + str(version))
        ...
        self.score_map_dir = os.path.join(self.result_dir.replace('tracker_result', 'score_map_json'), 'score_map')
    ...
    def run_unsupervised(self, tracker, visualize=False):
        ...
        for s, (img_files, anno) in enumerate(self.dataset):
            ...
            # skip if results exist
            record_file = os.path.join(
                self.result_dir, tracker.name + 'tune7', '%s.txt' % seq_name)  # 路径修改
            if os.path.exists(record_file):
                print('  Found results, skipping', seq_name)
                continue
    ...
    def run_for_score_map(self, tracker):
        ...
        for s, (img_files, anno) in enumerate(self.dataset):
            ...
            # skip if results exist
            num_file = os.path.join(self.score_map_dir, f'{seq_name}_score_map.json')
            ...
            # tracking loop
            seq_score_map_data = tracker.track_for_score_map(img_files, anno_rects)  # 对齐run_sequence
    ...
    def sequences_select(self, tracker_names, save_dir):
        assert isinstance(tracker_names, (list, tuple))

        if not os.path.isdir(save_dir):
            os.makedirs(save_dir)

        info_file = os.path.join(save_dir, 'data', self.dataset.name, 'list.txt')
        seq_names = []
        for s, (_, anno) in enumerate(self.dataset):  # 序列遍历
            ...
            for i, tracker in enumerate(tracker_names):  # tracker遍历
                ...
                num_csore_json_file = os.path.join(save_dir, 'score_map_json', tracker, 'vot/test', self.dataset.name,
                                                   'score_map', f'{seq_name}_score_map.json')
                if os.path.exists(num_csore_json_file):
                    print('operate %s in vot of %s' % (seq_name, tracker))
                else:
                ...
                with open(num_csore_json_file, "r") as f:
                    data = json.load(f)
                _, _, seq_tc_num[i] = extract_candidate_set(data, anno_rects)

            for j in range(len(anno_rects) - 1):
                if np.any(np.isnan(anno_rects[j + 1])):
            ...
            if len(soi_frames) > 0:  # 存在soi
                self.choose_seq_for_soi(seq_name, soi_frames, save_dir)
                # append
    ...
    def choose_seq_for_soi(self, seq_name, soi_frames, save_dir):
        file_path = self.dataset.seq_dirs[0]
