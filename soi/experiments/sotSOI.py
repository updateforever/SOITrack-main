...
from ..datasets import votSOI
# from ..datasets import otbSOI
from ..utils.metrics import rect_iou, center_error, normalized_center_error
from ..utils.help import makedir


def call_back():
    return ExperimentSOT_SOI


class ExperimentSOT_SOI(ExperimentBASE):
    ...
    def __init__(self, root_dir, save_dir, subset='test', return_meta=False, th=0.1, list_file=None):
        ...
        self.dataset = SOT_SOI(root_dir, subset, return_meta=return_meta,
    ...
    def report(self, tracker_names, th=0.1):
        ...
        for name in tracker_names:
            ...
            for s, (_, anno) in enumerate(self.dataset):
                ...
                if name == str('TransKT') or name == str('ToMP') or name == str('KeepTrack'):
        ...
        self.plot_curves(tracker_names, th=th)
    ...
    def plot_curves(self, tracker_names, extension='.png', th=0.1):
        # assume tracker_names[0] is your tracker
        report_dir = os.path.join(self.report_dir)
        ...
        ax.set(xlabel='Overlap threshold',
               ylabel='Success rate',
               xlim=(0, 1), ylim=(0, 1),
               title='Success plots on LaSOT_SOI')
        ax.grid(True)
        fig.tight_layout()
        ...
        ax.set(xlabel='Location error threshold',
               ...
               title='Precision plots on LaSOT-SOI')
        ax.grid(True)
        fig.tight_layout()
        ...
        ax.set(xlabel='Normalized location error threshold',
               ...
               xlim=(0, thr_ce.max()), ylim=(0, 1),
               title='Normalized precision plots on LaSOT-SOI')
        ax.grid(True)
        fig.tight_layout()
